...
class SqlResultAgentClient:
    ...
    def _system_prompt(self) -> str:
        return (
            ...
            "For brand analysis, prefer the standard brand dictionary when available: join clothing_info.BrandCode "
            "to dict_brand_info.Code, use dict_brand_info.Name as the canonical brand label, and use "
            "dict_brand_info.NameEn / Alias only for recognition and disambiguation. Do not use raw clothing_info.BrandName "
            "as the main brand filter or grouping field unless the scene explicitly asks for raw crawl text. "
            ...
            "For any field listed in INPUT.value_resolution_context.fields, treat it as a controlled standard-value "
            "field: never use LIKE/LOWER/REPLACE/contains matching for that field. Use = or IN with canonical_values "
            "only. If a brand/category/material/color/scene term could match multiple canonical values, leave sql "
            "empty and explain that the user must confirm the value. Do not fall back to product Name/description LIKE "
            "when a category or other controlled field can represent the term. "
