...
def _template_base_url() -> str:
    return (os.getenv("NEXT_PUBLIC_URL") or "http://localhost").strip().rstrip("/")
...
async def get_layout_by_name(layout_name: str) -> PresentationLayoutModel:
    ...
    template_base_url = _template_base_url()
    url = f"{template_base_url}/schema?{query}"
    ...
    if schema_payload is None:
        fallback_error = None
        fallback_url = f"{template_base_url}/api/template?group={layout_name}"
        LOGGER.info(
