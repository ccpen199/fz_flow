...
DIRECT_CONTENT_START = "PRESENTON_DIRECT_CONTENT_JSON_START"
DIRECT_CONTENT_END = "PRESENTON_DIRECT_CONTENT_JSON_END"


def _extract_direct_content(outline: str) -> Optional[dict]:
    start = outline.find(DIRECT_CONTENT_START)
    end = outline.find(DIRECT_CONTENT_END)
    if start == -1 or end == -1 or end <= start:
        return None

    raw_json = outline[start + len(DIRECT_CONTENT_START) : end].strip()
    try:
...
def _direct_speaker_note(outline: str) -> str:
    clean_lines = [
        line.strip()
        for line in outline.splitlines()
        if line.strip()
        and "PRESENTON_LAYOUT_ID:" not in line
        and DIRECT_CONTENT_START not in line
        and DIRECT_CONTENT_END not in line
    ]
    note = " ".join(clean_lines[:6])
    if len(note) < 100:
        note = (
            note
            + " 本页由系统根据结构化 SQL 结果注入 Presenton 模板生成，图表数据来自查询结果聚合，"
            + "用于快速形成可汇报的数据分析页面。"
        )
    return note[:500]
...
async def get_slide_content_from_type_and_outline(
    ...
    direct_content = _extract_direct_content(outline.content)
    if direct_content is not None:
        direct_content.setdefault("__speaker_note__", _direct_speaker_note(outline.content))
