...
def get_generate_kwargs(
    ...
    if response_format is not None and not any("json" in str(getattr(message, "content", "")).lower() for message in effective_messages):
        effective_messages.append(UserMessage(content="Return valid JSON only."))
    kwargs: dict[str, Any] = {
        "model": model,
